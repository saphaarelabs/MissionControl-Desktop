...
import { desktopConfig, isDesktopApp } from '../lib/desktop';
    ...
    const handleGoogle = async () => {
        ...
        try {
            ...
            await signUp.authenticateWithRedirect({
                ...
                redirectUrl: isDesktopApp
                    ? `${desktopConfig().protocol}://sso-callback`
                    : '/sso-callback',
                redirectUrlComplete: isDesktopApp
                    ? `${desktopConfig().protocol}://sso-callback?oauth_complete=1&intent=sign-up`
                    : '/sso-callback?oauth_complete=1&intent=sign-up',
            });
            if (isDesktopApp) {
                setInfo('We opened account creation in your browser. Once it finishes, the desktop app will continue automatically.');
            }
        } catch (err) {
